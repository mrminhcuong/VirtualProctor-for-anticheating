...
import csv
import tqdm 
from data import BodyPart, Person, config
import cv2


detector = DaisykitPoseDetector(config)
 
# def serialdetect(input_image, inference_count=3):
#     """takes image input as numpy array"""
...
#     and save those keypoints in a csv file for the later use in the classification task 
        ...
        def __init__(self, images_in_folder, csvs_out_path):
            ...
            self._csvs_out_folder_per_class = 'csv_posedetection'
            self._message = []
...
images_in_folder = os.path.join('dataset', 'train')
